...
// malloc method. Procedure inccludes checking space, then assign the block if space is
// fit, in order to avoid wasting, split the block if the space if needed. Then update the tag
// for both current block and following block.
void* mm_malloc (size_t size) {
  ...
  // alloac the block if space is good, split the block if size is too big
  if((blockSize - reqSize) < MIN_BLOCK_SIZE){
    ...
    *((int*)UNSCALED_POINTER_ADD(ptrFreeBlock, (reqSize-WORD_SIZE)))= reqSize;
    ...
    //update info of the remaining block
    BlockInfo * followingBlock = (BlockInfo *) UNSCALED_POINTER_ADD(ptrFreeBlock,reqSize);
    followingBlock->sizeAndTags = (((blockSize - reqSize) | TAG_PRECEDING_USED) & (~TAG_USED));
    //change the boundary for the remained block
    *((int*)UNSCALED_POINTER_ADD(ptrFreeBlock, (blockSize-WORD_SIZE))) = blockSize-reqSize;
    //add the free block
    insertFreeBlock(followingBlock);
  }
  //mark the used tag of the block
  ptrFreeBlock->sizeAndTags |= TAG_USED;

  return ((void*)UNSCALED_POINTER_ADD(ptrFreeBlock,WORD_SIZE)); }

/* Free the block referenced by ptr. */
// implement the free method,procedures includes getting the current block info
// and then get the payload size, change the tags for the current block and the following block 
void mm_free (void *ptr) {
  ...
  //first move the pointer to the header
  blockInfo = (BlockInfo *)UNSCALED_POINTER_SUB(ptr,WORD_SIZE);
  //stop if the block is not asigned by the malloc
  if(((blockInfo->sizeAndTags) & (TAG_USED)) == 0){
    printf("pointer is not given by malloc");
    return;
  }
  ...
  //mark the next block's previous used tag to unused
